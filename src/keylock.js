...
function makeHashili(str) {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    output += consonant[Math.floor(remainder / 5)] +
              vowel[remainder % 5];
...
function acceptpwd(){
	...
	chrome.runtime.sendMessage({message: 'preserve_keys', KeyStr: KeyStr, myKey: myKey, myLockbin: myLockbin, myLock: myLock, myezLock: myezLock, locDir: locDir});		//send them to background
